...
function AI(name, options) {
    ...
    var
        slave
        , thinkTimeout
        , moveTimeout = options.moveTimeout + _.random(75, 150)
        ;
    ...
    function start(slave) {
        var
            previousLocation = slave.location
            ...
            , doThink = function doThink() {
                ...
                var
                    ...
                    , makeNextThink = function () {
                        ...
                        thinkTimeout = setTimeout(doThink, moveTimeout);
                    }
                    ;
        ...
        thinkTimeout = setTimeout(function _fn() {
            ...
            if (path && (newPathPoint = path.shift())) {
                ...
                try {
                    ...
                    thinkTimeout = setTimeout(_fn, moveTimeout);
                }
            } else {
                thinkTimeout = setTimeout(callback, options.bomb.detonationTimeout - 3 * moveTimeout);
            }
        }, moveTimeout);
    }
    ...
    function calcWeight(item, location) {
        ...
        if (item instanceof Block) {
            if (location.x % 2 + location.y % 2 == 1) {
                return options.bomb.detonationTimeout + 4 * moveTimeout + options.place.flameLifetime;
            } else {
                return options.bomb.detonationTimeout + 3 * moveTimeout + options.place.flameLifetime;
            }
        } else if (item instanceof Bomb) {
            if (location.x % 2 + location.y % 2 == 1) {
                return options.bomb.detonationTimeout + 3 * moveTimeout + options.place.flameLifetime;
            } else {
                return options.bomb.detonationTimeout + 5 * moveTimeout + options.place.flameLifetime;
            }
